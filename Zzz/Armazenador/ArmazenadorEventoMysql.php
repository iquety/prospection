...
use Comum\Infraestrutura\Evento\ArmazenadorEventoMysql\Consultador;
use Comum\Infraestrutura\Evento\ArmazenadorEventoMysql\Registrador;
...
class ArmazenadorEventoMysql implements ArmazenadorEvento
{
    ...
    public function listarRegistrosMaterializacao(
        ...
        $eventosPorAgregado = $this->consultador->listaEventosParaRegistros(
            $this->consultador->listaRegistrosPorData($agregado::rotulo(), $momentoInicial, $intervalo)
        );
        ...
        foreach ($eventosPorAgregado as $registro) {
            ...
            if (isset($eventosAgrupados[$idAgregado]) === false) {
                $ocorrenciasAgrupadas[$idAgregado] = [];
                $eventosAgrupados[$idAgregado] = [];
            }

            $ocorrenciasAgrupadas[$idAgregado][] = new DataHora($registro['ocorrido_em']);

            $eventosAgrupados[$idAgregado][] = $this->empacotador
                ->desempacotar($agregado, $registro['rotulo_evento'], $registro['dados']);
        }

        $lista = [];

        foreach ($eventosAgrupados as $idAgregado => $listaEventos) {
            $classeAgregado = $agregado::class;

            /** @var EntidadeRaiz $entidade */
            $entidade = new $classeAgregado();

            $entidade->estado()->setarDataCriacao($ocorrenciasAgrupadas[$idAgregado][0]);

            foreach ($listaEventos as $indice => $evento) {
                $entidade->consolidarEstado([ $evento ]);
                $entidade->estado()->setarDataAlteracao($ocorrenciasAgrupadas[$idAgregado][$indice]);
            }
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // Exclusivos ArmazenadorEventoMysql
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

    public function criarTabelaEventos(): void
    {
        $this->registrador->criarTabela();
    }

    public function removerTabelaEventos(): void
    {
        $this->registrador->removerTabela();
    }

    public function ultimoErro(): Erro
    {
        return $this->registrador->ultimoErro();
    }
}
